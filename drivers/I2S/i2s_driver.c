...
static int __init i2s_init(void)
{
    ...
    ret = alloc_chrdev_region(&dev, 0, 1, DEVICE_NAME);// It allocates device numbers (both major and minor) and stores it in dev_t structure 
    if (ret < 0) {
...
MODULE_AUTHOR("YourName");           // Author of the module
