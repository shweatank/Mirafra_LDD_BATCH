#include <linux/module.h>        // Essential for creating a Linux kernel module
#include <linux/init.h>           // Defines macros related to module initialization and cleanup
#include <linux/kernel.h>         // Provides macros and functions for kernel logging (e.g., printk)
#include <linux/fs.h>             // For managing file systems and device operations (e.g., register_chrdev)
#include <linux/io.h>             // For memory-mapped I/O operations (e.g., ioremap, ioread32, iowrite32)
#include <linux/interrupt.h>      // For interrupt handling (e.g., request_irq, free_irq)
#include <linux/slab.h>           // For dynamic memory allocation using the slab allocator (e.g., kmalloc, kfree)
#include <linux/dma-mapping.h>    // For DMA buffer management (e.g., dma_alloc_coherent, dma_free_coherent)
#include <linux/uaccess.h>        // For copying data between user space and kernel space (e.g., copy_from_user, copy_to_user)
#include <linux/cdev.h>           // For managing character devices (e.g., cdev_init, cdev_add)
#include <linux/types.h>          // For fixed-width integer types (e.g., uint32_t)

#define DEVICE_NAME "uart_dma"
//uart registers
#define UART_BASE 0xFE201000  // UART base address for Raspberry Pi 4
#define UART_DR_OFFSET 0x00  // Data register offset
#define UART_DMACR_OFFSET 0x48 // DMA control register offset
//dma registers
#define DMA_BASE 0xFE007000  // DMA base address for Raspberry Pi 4
...
Points to the DMA controller memory-mapped register base.

static void __iomem *uart_regs;//Points to the UART memory-mapped(virtual space) register base.
static void __iomem *dma_regs;//Points to the DMA controller memory-mapped register base.

static struct dma_cb *dma_cb;

static dma_addr_t dma_cb_handle, dma_buffer_handle;// used to store physical memory addresses for DMA (Direct Memory Access) buffers and control blocks.
...
static ssize_t uart_dma_write(struct file *file, const char __user *buf, size_t count, loff_t *pos) {
    ...
    //Write the physical address of the DMA control block to the DMA control block address register
// This tells the DMA controller where to find the DMA control block that contains the transfer configuration.
...
static int __init uart_dma_init(void) {
    int irq = 57; // Correct IRQ number for UART DMA on Raspberry Pi 4

    printk(KERN_INFO "Initializing UART-DMA driver\n");

    // Map UART and DMA registers
    uart_regs = ioremap(UART_BASE, 0x100);//256 bytes is enough for UART register space.
    dma_regs = ioremap(DMA_BASE, 0x1000);//4096 bytes is used to cover the broader range of DMA registers.
                                         //
//checks whether the UART and DMA registers were successfully mapped to virtual memory
    if (!uart_regs || !dma_regs) {//on failure returns 0.
    ...
    /*
     NULL since it's not device-specific
     Size of the buffer to allocate (PAGE_SIZE is typically 4KB)
    dma_handle: Pointer to store the physical address of the allocated memory
     gfp_flags: Allocation flags (GFP_KERNEL means the allocation can block
     */
...
/* IRQ number to be requested
 Interrupt handler function
 Flag indicating the IRQ is shared between multiple handlers
 Identifier for the device (used for interrupt management)
 Pointer to store the device's major number*/
...
MODULE_DESCRIPTION("Enhanced UART DMA Driver for Raspberry Pi 4 ");
